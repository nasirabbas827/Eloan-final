...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    // Fetch the current loan application details
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($documents as $document) {
        if ($_FILES[$document]["error"] == 0 && $_FILES[$document]["size"] > 0) {
            ...
            // Keep the existing file names if no new file is uploaded
            $uploaded_files[$document] = $application[$document];
        }
    }
    ...
    // If no errors, update the loan application in the database
    if (empty($loan_amount_err) && empty($loan_type_err) && empty($purpose_err) && empty($repayment_period_err) && empty($document_err)) {

        // Update loan application data in the database
        $sql = "UPDATE loan_applications SET loan_amount = ?, loan_type = ?, purpose = ?, repayment_period = ?, 
                proof_of_income = ?, proof_of_identification = ?, credit_history = ?, application_status = ? WHERE id = ? AND user_id = ?";
        if ($stmt = mysqli_prepare($conn, $sql)) {
            mysqli_stmt_bind_param($stmt, "ssssssssss", $loan_amount, $loan_type, $purpose, $repayment_period, 
                $uploaded_files['proof_of_income'], $uploaded_files['proof_of_identification'], 
                $uploaded_files['credit_history'], $application_status, $application_id, $_SESSION["id"]);
            if (mysqli_stmt_execute($stmt)) {
                ...
                echo "Error: Could not update the loan application.";
            }
            mysqli_stmt_close($stmt);
        }
    }
...
<head>
    ...
    <title>Edit Loan Application - Borrower Dashboard</title>
...
<body>
    ...
    <div class="container mt-5 mb-5">
        <div class="card mx-auto" style="max-width: 600px;">
            <div class="card-body">
                <h1 class="text-center mb-5">Edit Loan Application</h1>

                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . "?id=" . $application_id; ?>" method="post" enctype="multipart/form-data">
                    <!-- Loan Amount -->
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="file" class="form-control" id="proof_of_income" name="proof_of_income">
                    </div>

                    <div class="mb-3">
                        <label for="proof_of_identification" class="form-label">Upload Proof of Identification</label>
                        <input type="file" class="form-control" id="proof_of_identification" name="proof_of_identification">
                    </div>

                    <div class="mb-3">
                        <label for="credit_history" class="form-label">Upload Credit History</label>
                        <input type="file" class="form-control" id="credit_history" name="credit_history">
                    </div>
